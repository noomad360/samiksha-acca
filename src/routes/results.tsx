...
import { CheckCircle2, XCircle, ChevronDown, Trophy, RotateCcw, FileText } from "lucide-react";

export const Route = createFileRoute("/results")({
  head: () => ({
    meta: [{ title: "Your results — ACCA FA Mock" }, { name: "robots", content: "noindex" }],
  }),
...
function ResultsPage() {
  ...
  const summary = useMemo(() => {
    if (!data) return null;
    let correct = 0;
    const autoMarkedQuestions = questions.filter((q) => q.type !== "long");
    autoMarkedQuestions.forEach((q) => {
      if (isCorrect(q.id, data.answers[q.id])) correct += 1;
    });
    const marks = correct * 2;
    const total = autoMarkedQuestions.reduce((sum, q) => sum + (q.marks ?? 2), 0);
          ...
          <div className="bg-gradient-brand px-6 py-8 text-brand-foreground">
            ...
            <div className="mt-3 flex items-end gap-3">
              ...
              <div className="pb-2 text-sm opacity-90">
                {summary.marks} / {summary.total} auto-marked marks
              </div>
            </div>
            <div className="mt-2 text-sm opacity-90">
              {summary.correct} of 35 Section A questions correct · {minutes}m {seconds}s used
            </div>
          </div>
          <div className="flex items-center justify-between gap-3 px-6 py-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-muted-foreground">
                Section B long answers are included for manual review.
              </span>
            </div>
            <Link to="/">
...
function formatAnswer(q: ReturnType<typeof questions.find> & {}, a: StoredAnswer): string {
  ...
  if (q.type === "long") return String(a);
      ...
      >
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            <span>Q{q.id}</span>
            <span>·</span>
            <span className={manual ? "text-brand" : correct ? "text-success" : "text-destructive"}>
              {manual ? "Manual review" : correct ? "Correct" : "Incorrect"}
            </span>
          </div>
          <div className="mt-1 line-clamp-2 text-sm">{"title" in q ? q.title : q.prompt}</div>
        </div>
        <ChevronDown
        ...
        <div className="border-t px-5 py-4 text-sm">
          ...
          {"requirements" in q && (
            <div className="mt-4 space-y-2 rounded-md border bg-background p-3">
              ...
              {q.requirements.map((requirement) => (
                <div key={requirement.label} className="text-sm">
                  <span className="font-semibold">{requirement.label}</span>{" "}
                  <span>{requirement.text}</span>{" "}
                  <span className="text-muted-foreground">[{requirement.marks} marks]</span>
                </div>
              ))}
            </div>
          )}
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-md border bg-background p-3">
              ...
              <div
                className={`mt-1 whitespace-pre-line ${manual ? "" : correct ? "text-success" : "text-destructive"}`}
              >
            ...
            <div className="rounded-md border bg-background p-3">
              <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                {manual ? "Marking guide" : "Correct answer"}
              </div>
              <div className="mt-1 whitespace-pre-line text-success">{correctAnswerText(q)}</div>
            </div>
          </div>
          {q.type !== "long" && (
